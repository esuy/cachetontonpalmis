...
public class BaseCoverFlow extends Gallery {
    ...
    public BaseCoverFlow(Context context) {
        super(context);
        this.initialize();
    }
    ...
    public BaseCoverFlow(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void applyXmlAttributes(AttributeSet attrs) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.BaseCoverFlow);

        this.actionDistance = a.getInteger(R.styleable.BaseCoverFlow_actionDistance, ACTION_DISTANCE_AUTO);
        this.scaleDownGravity = a.getFloat(R.styleable.BaseCoverFlow_scaleDownGravity, 1.0f);
        this.maxRotation = a.getInteger(R.styleable.BaseCoverFlow_maxRotation, 45);
        this.unselectedAlpha = a.getFloat(R.styleable.BaseCoverFlow_unselectedAlpha, 0.3f);
        this.unselectedSaturation = a.getFloat(R.styleable.BaseCoverFlow_unselectedSaturation, 0.0f);
        this.unselectedScale = a.getFloat(R.styleable.BaseCoverFlow_unselectedScale, 0.75f);
    }
    ...
    public void setReflectionRatio(float reflectionRatio) {
        ...
        if (this.getAdapter() != null) {
            ((BaseCoverFlowAdapter) this.getAdapter()).notifyDataSetChanged();
        }
    }
    ...
    /**
     * Use this to provide a {@link BaseCoverFlowAdapter} to the coverflow. This
     * method will throw an {@link ClassCastException} if the passed adapter does not
     * subclass {@link BaseCoverFlowAdapter}.
     *
     * @param adapter
     */
    @Override
    public void setAdapter(SpinnerAdapter adapter) {
        if (!(adapter instanceof BaseCoverFlowAdapter)) {
            throw new ClassCastException(BaseCoverFlow.class.getSimpleName() + " only works in conjunction with a " + BaseCoverFlowAdapter.class.getSimpleName());
        }
    ...
    protected boolean getChildStaticTransformation(View child, Transformation t) {
        ...
        BaseCoverFlowItemWrapper item = (BaseCoverFlowItemWrapper) child;
